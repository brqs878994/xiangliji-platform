...
import './publish.scss';
import { createDraft, fallbackCategories, getCategories, submitDraftReview, type PlatformCategory, updateDraft } from '../../services/platform';

type Draft = { category: string; title: string; location: string; validity: string };
const validityOptions = ['1 天', '3 天', '7 天', '14 天', '30 天'];
const validityDays = (value: string) => Number.parseInt(value, 10) || 7;
...
export default function PublishPage() {
  ...
  const [validityIndex, setValidityIndex] = useState(2);
  ...
  async function saveDraft() {
    ...
    try {
      ...
      const payload = { userId: 'user-demo', title: draft.title || content.trim().slice(0, 28), category, townCode: 'chengguan', body: content.trim(), validDays: validityDays(draft.validity) };
      const result = draftId ? await updateDraft(draftId, payload) : await createDraft(payload);
      if (result && !draftId) setDraftId(result.id);
      setStep(2);
      notify(draftId ? '草稿已更新' : '草稿已保存，现在可以提交审核');
  ...
  const reviewAction = step === 1 ? makeDraft : step === 2 && !draftId ? saveDraft : step === 2 ? submitReview : () => undefined;
  const reviewLabel = step === 1 ? '先看一眼草稿' : step === 2 && !draftId ? '保存草稿' : step === 2 ? '提交并发布' : '已发布';

...
import './explore.scss';
import { fallbackCategories, fallbackPosts, getCategories, getPosts, type PlatformCategory, type PlatformPost } from '../../services/platform';
...
export default function ExplorePage() {
  ...
  const [posts, setPosts] = useState<PlatformPost[]>(fallbackPosts);
  const [categories, setCategories] = useState<PlatformCategory[]>(fallbackCategories);

  useEffect(() => {
    void getPosts({ townCode: 'chengguan', category: initialCategory === '全部' ? undefined : initialCategory }).then(setPosts);

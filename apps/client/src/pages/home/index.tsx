...
import { getPosts, getResponses, toggleFavorite } from '../../services/platform';
...
const initialPosts = [
  { id: 'post-corn-001', type: '助农供求', age: '同镇 · 12 分钟前', title: '收土鸡蛋，长期要货', summary: '每周稳定收 200 斤，个头均匀，城关镇可上门看货。', price: '1.20', unit: '元/斤', response: '2 人已联系', label: '收', tone: 'market', image: marketBasket },
  { id: 'post-machine-001', type: '二手市场', age: '邻村 · 1 小时前', title: '旧打谷机，正常使用', summary: '有使用痕迹，配件齐全，王家镇自提。', price: '1200', unit: '元 · 可议价', response: '1 人已收藏', label: '卖', tone: 'tools' },
  { id: 'post-loading-001', type: '日结零工', age: '城关镇 · 2 小时前', title: '下午装车，缺 5 人', summary: '今天 14:00 集合，做完结算，管一顿午饭。', price: '260', unit: '元/天', response: '还缺 3 人', label: '招', tone: 'job' },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    void getPosts({ townCode: 'chengguan' }).then((items) => {
      const nextPosts = items.slice(0, 3).map((post, index) => ({
        id: post.id,
        type: post.category,
  ...
  async function handleFavorite(postId: string) {
    ...
    setFavoriteIds((current) => {
      ...
      if (saved) next.delete(postId); else next.add(postId);
      return next;
    });
    try {
      await toggleFavorite(postId, currentUserId, saved);
      showNotice(saved ? '已取消收藏' : '已收藏');
    } catch {
      setFavoriteIds((current) => {
        const next = new Set(current);
      ...
      <View className='main-content'>
        ...
        <View className='hero-grid'>
          <View className={`voice-card ${textMode ? 'typing-open' : ''}`}>
            <View className='voice-card-top'><View className='live-label'><Text className='live-dot' />本镇信号在线</View><View className='voice-mode-actions'><Text className='voice-hint'>说一句，帮你查找或提问</Text><View className='mode-toggle' onClick={() => setTextMode(!textMode)}><Text className='icon'>▤</Text><Text>{textMode ? '切回语音' : '改用文字输入'}</Text></View></View></View>
